...
def detect_speaker_segments(video_path, frame_skip=2, min_segment_dur=1.0, pad=0.15, resize_width=960):
...
def extract_speaker_clips(video_path, output_folder="output_clips", frame_skip=2, min_segment_dur=1.0,
                          threads=4, preset="ultrafast", silence_thresh_db=-45, min_silence_len_ms=400,
                          keep_silence_ms=100):
    ...
    for i, (start, end) in enumerate(merged):
        ...
        clip_path = os.path.join(output_folder, f"speaker_clip_{i+1}.mp4")
        subclip.write_videofile(clip_path, codec="libx264", audio_codec="aac", preset=preset, threads=threads, logger=None)
...
def build_ui():
    with gr.Blocks() as demo:
        ...
        with gr.Accordion("Advanced Settings", open=False):
            ...
            min_segment_dur = gr.Slider(0.5, 5.0, value=1.0)
